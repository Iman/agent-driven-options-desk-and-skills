...
Generated by `scripts/inventory.py` on 2026-09-03. Do not edit by hand: run `python3 scripts/refresh.py` and it is rewritten.
...
- **diagram()** (line 123): The whole figure as one SVG element
- **panel()** (line 154): The pipeline panel: figure, hint and caption
...
- **positioning()** (line 140): Dealer gamma exposure, the walls, the flip and max pain
- **volatility()** (line 177): The smile, the expected move, and the premium over realised
- **structures()** (line 213): The payoff engine, the lognormal law behind P(profit), and friction
- **time_spreads()** (line 256): Structures marked at the near expiry, with the two ratio columns
- **simulation()** (line 290): GARCH-t by MCMC, the paths, and the two tail figures
- **backtest()** (line 351): Modelled premiums on real closes, and the block-aware tests
...
- **render()** (line 1223): Render the complete dashboard document from one payload
...
- **handle()** (line 509): Handle one JSON-RPC request. Returns a response dict, or None for a
- **serve()** (line 621): Read line-delimited JSON-RPC from stdin, write responses to stdout
- **main()** (line 654): Serve the JSON-RPC loop on stdin and stdout until the client closes it
...
- class **Provider** (line 45): Base class. Subclasses set the attributes and implement what they
  - methods: `access_status`, `require_access`, `available`, `describe`, `option_chain`, `underlying_quote`, `risk_free_rate`, `underlying_history`
...
Roughly 2250 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `scripts/check_artifact.py`, `workflows/pull-and-grade.md`.
...
Roughly 1756 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/read-the-book.md`.
...
Roughly 1855 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/choose-a-structure.md`.
...
### shell/tests (437 test functions)
...
- `shell/tests/test_dashboard_maths.py`: 11
- `shell/tests/test_dashboard_page.py`: 39
...
- `shell/tests/test_mcp_server.py`: 40
- `shell/tests/test_packaging.py`: 20
- `shell/tests/test_providers.py`: 12
...
60 modules, 16175 lines of source, 178 public functions, 15 public classes, 774 test functions.
