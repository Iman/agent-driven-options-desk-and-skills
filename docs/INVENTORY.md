...
Generated by `scripts/inventory.py` on 2026-09-10. Do not edit by hand: run `python3 scripts/refresh.py` and it is rewritten.
...
- **realised_volatility()** (line 43): Annualised volatility from log returns, or None if too few
- **synthetic_chain()** (line 52): A chain of strikes around spot, priced by the model
- **run_backtest()** (line 104): Enter a structure repeatedly and settle each one at expiry
...
- **permutation_p_value()** (line 97): How often a no-edge rule beats this mean by chance
- **bootstrap_mean_interval()** (line 154): Confidence interval for the mean return, by resampling trades
...
- **d1_d2()** (line 106): The two Black-Scholes arguments, shared by price and every Greek
- **bs_price()** (line 113): Black-Scholes-Merton price of a European option
- **vega_raw()** (line 136): dV/dsigma per 1.00 of volatility. Used by the IV solver
- **intrinsic()** (line 143): Discounted intrinsic value, the no-arbitrage floor for a European
- **implied_vol()** (line 153): Implied volatility from an option price, or None
...
- **terminal_risk()** (line 177): Value at risk and expected shortfall on the horizon return
- **position_distribution()** (line 225): Distribution of a position's profit across the simulated terminals
...
- class **TimeLeg** (line 55): One leg of a structure whose legs expire on different dates
  - methods: `__init__`, `value_at`, `as_dict`
- **net_option_cash()** (line 128): Cash at entry. Positive is a credit, negative a debit
- **pnl_at()** (line 133): Profit once at_days have passed, with the underlying at price
- **payoff_curve()** (line 140): (prices, profits) at the mark date, with strikes included exactly
- **analyze_at_front()** (line 156): Risk graph at the near expiry
- **probability_at_front()** (line 265): Model probability of profit at the near expiry, or None
- **calendar_spread()** (line 430): Sell the near expiry and buy the far one at the same strike
- **diagonal_spread()** (line 476): A calendar with different strikes: carry plus a directional lean
- **ratio_call_diagonal()** (line 653): Bullish ratio diagonal. kind is fixed by the name
- **ratio_put_diagonal()** (line 658): Bearish mirror of the ratio call diagonal
- **build_time_spread()** (line 671): Build a two-expiry structure by name
...
- **build_parser()** (line 36): Assemble the top level parser by asking each command module to register
- **main()** (line 106): Dispatch argv to the named subcommand and return its exit code
...
- **run()** (line 74): Enter one structure repeatedly across real history and write a backtest
- **main()** (line 215): Parse argv for this command alone and run it, so the command works when
...
- **snapshot_schema()** (line 45): Describe the private user-data contract for agent clients
- **add_arguments()** (line 343): Register the pull options: expiry, provider, risk free rate, dividend
- **run()** (line 574): Pull one option chain, solve implied volatility per contract where the
- **main()** (line 862): Parse argv for this command alone and run it, so the command works when
...
### `shell/src/optiondesk/cli/plots.py`

Create chat-ready PNG plots from an option-chain snapshot.

180 lines.

- **add_arguments()** (line 17): undocumented
- **run()** (line 83): Fetch or read a chain, calculate available analytics, and write PNGs
- **main()** (line 176): undocumented

Undocumented public names in this module: add_arguments, main.
...
- **main()** (line 319): Parse argv for this command alone and run it, so the command works when
...
- **playbook_when()** (line 339): The playbook's own note on when a structure is the right one, or an
- **run()** (line 434): Build one named structure, or recommend one from the stated view, and
- **main()** (line 625): Parse argv for this command alone and run it, so the command works when
...
- **setting()** (line 59): Resolve one setting through the documented precedence chain
- **artifact_dir()** (line 72): Directory artifacts are written to. Created on demand by the writer
- **public_data_mode()** (line 78): Return the data boundary selected for this process
- **provider_key()** (line 90): API key for a provider, or None when it needs none or has none set
- **has_key()** (line 98): True when a provider that requires a key has one available
- **configured_providers()** (line 106): Report which providers could answer, without revealing any secret
...
- **index()** (line 39): Every (underlying, expiry) group on disk, newest group first
- **select()** (line 96): The requested group, or the most recently touched one
- **forward_ledger()** (line 117): The forward ledger, or None when no position has ever been opened
- **collect()** (line 129): Everything the page needs, for one selected group
- **volatility_surface()** (line 212): Implied volatility by strike and expiry, from every chain on disk
- **variance_premium()** (line 263): At-the-money implied volatility against the volatility shown
- **condor_candidates()** (line 299): Structures with two short strikes, with the width between them
- **chain_series()** (line 358): Volume and open interest by strike, straight from the snapshot
- **ladder_series()** (line 381): Split a ladder into call and put series for plotting

### `shell/src/optiondesk/dashboard/flow.py`

The pipeline as one figure: which command reads what and writes what.

175 lines.

- **diagram()** (line 129): The whole figure as one SVG element
- **panel()** (line 166): The pipeline panel: figure, hint and caption

### `shell/src/optiondesk/dashboard/maths.py`

The arithmetic behind each panel, printed beside the panel.

448 lines.

- **constants()** (line 27): Every number the blocks quote, read from the engine
- **block()** (line 74): Wrap one block of plain-text arithmetic as a collapsible note
- **pricing()** (line 90): The model behind every price and Greek on the page
- **positioning()** (line 144): Dealer gamma exposure, the walls, the flip and max pain
- **volatility()** (line 186): The smile, the expected move, and the premium over realised
- **structures()** (line 227): The payoff engine, the lognormal law behind P(profit), and friction
- **time_spreads()** (line 273): Structures marked at the near expiry, with the two ratio columns
- **simulation()** (line 307): GARCH-t by MCMC, the paths, and the two tail figures
- **backtest()** (line 389): Modelled premiums on real closes, and the block-aware tests
...
- **render()** (line 1318): Render the complete dashboard document from one payload
...
- class **EngineUnavailable** (line 50): Raised when analytics are requested and the engine is absent
  - methods: `__init__`
- **require()** (line 57): Return the engine module surface, or raise with a fixable message
- **strategies()** (line 70): The engine's strategy surface: playbook, payoff, friction
- **analytics()** (line 84): The engine's chain analytics surface
- **simulation()** (line 97): The engine's simulation surface: GARCH-t posterior and paths
- **backtest()** (line 104): The engine's backtest surface: the runner and its statistics
- **pricing()** (line 111): The engine's pricing surface: the Black-Scholes model and the
- **status()** (line 123): Describe engine availability for artifacts and for tools/list
...
- **handle()** (line 672): Handle one JSON-RPC request. Returns a response dict, or None for a
- **serve()** (line 785): Read line-delimited JSON-RPC from stdin, write responses to stdout
- **main()** (line 818): Serve the JSON-RPC loop on stdin and stdout until the client closes it

### `shell/src/optiondesk/plotting.py`

Dependency-free PNG charts for MCP clients that can display images.

373 lines.

- class **Canvas** (line 80): Small RGB raster with the primitives used by the chart panels
  - methods: `__init__`, `pixel`, `rect`, `line`, `text`, `png`
- **market_png()** (line 301): Render positioning, open interest, volume, and volatility smile
- **greeks_png()** (line 353): Render delta, gamma, theta, and vega from a Greek ladder
...
- **resolve()** (line 80): Return the provider that will answer, and why it was chosen
- **describe_all()** (line 140): Inventory for status reporting
...
- class **ProviderError** (line 28): Base class for provider failures
- class **ProviderUnavailable** (line 32): The provider cannot run: missing dependency, missing key, no network
- class **ProviderDataError** (line 40): The provider ran and returned something unusable
- class **Provider** (line 44): Base class. Subclasses set the attributes and implement what they
  - methods: `access_status`, `require_access`, `available`, `unavailable_reason`, `missing_dependency`, `describe`, `option_chain`, `underlying_quote`, `risk_free_rate`, `underlying_history`
...
- class **YahooProvider** (line 55): A local provider, with no key and an explicit acknowledgement
  - methods: `__init__`, `available`, `missing_dependency`, `underlying_quote`, `risk_free_rate`, `underlying_history`, `dividend_yield`, `expiries`, `option_chain`
...
- class **DeskState** (line 54): What flows through the graph
- **build_desk_graph()** (line 90): Compile the routine
- **open_desk()** (line 331): Run the routine to completion and return the final state
...
- **build_answer_prompt()** (line 43): A chat prompt that answers strictly from artifact context
- **build_router_prompt()** (line 57): A prompt for deciding which desk command answers a question
...
- **tool_specs()** (line 221): The tool definitions, without requiring LangChain to be installed
- **desk_tools()** (line 269): Build LangChain StructuredTools for the desk
...
Roughly 1509 tokens in SKILL.md. Bundled: nothing.
...
Roughly 1708 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/evaluate-a-rule.md`.
...
Roughly 2309 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `scripts/check_artifact.py`, `workflows/pull-and-grade.md`.
...
Roughly 1815 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/read-the-book.md`.
...
Roughly 1983 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/run-a-projection.md`.
...
Roughly 1915 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/choose-a-structure.md`.
...
- `shell/tests/test_chain_cli.py`: 29
...
- `shell/tests/test_dashboard_page.py`: 45
...
- `shell/tests/test_mcp_server.py`: 45
- `shell/tests/test_packaging.py`: 20
- `shell/tests/test_providers.py`: 21
- `shell/tests/test_run_script.py`: 1
- `shell/tests/test_runtime_docs.py`: 14
- `shell/tests/test_screenshots.py`: 3
- `shell/tests/test_simulate_cli.py`: 12
- `shell/tests/test_strategy_cli.py`: 20
- `shell/tests/test_summary_degraded_contract.py`: 4
- `shell/tests/test_wiki.py`: 3
- `shell/tests/test_yahoo_provider.py`: 26

### agent/tests (115 test functions)

- `agent/tests/test_artifacts.py`: 36
- `agent/tests/test_graph.py`: 39
...
60 modules, 16964 lines of source, 180 public functions, 15 public classes, 851 test functions.
