...
Generated by `scripts/inventory.py` on 2026-09-05. Do not edit by hand: run `python3 scripts/refresh.py` and it is rewritten.
...
- **add_arguments()** (line 327): Register the pull options: expiry, provider, risk free rate, dividend
- **run()** (line 518): Pull one option chain, solve implied volatility per contract where the
- **main()** (line 745): Parse argv for this command alone and run it, so the command works when
...
- **render()** (line 1300): Render the complete dashboard document from one payload
...
- class **ProviderError** (line 29): Base class for provider failures
- class **ProviderUnavailable** (line 33): The provider cannot run: missing dependency, missing key, no network
- class **ProviderDataError** (line 41): The provider ran and returned something unusable
- class **Provider** (line 45): Base class. Subclasses set the attributes and implement what they
...
Roughly 1044 tokens in SKILL.md. Bundled: nothing.
...
Roughly 1635 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/evaluate-a-rule.md`.
...
Roughly 1891 tokens in SKILL.md. Bundled: `.DS_Store`, `reference.md`, `workflows/run-a-projection.md`.
...
### shell/tests (460 test functions)
...
- `shell/tests/test_chain_cli.py`: 22
...
- `shell/tests/test_dashboard_page.py`: 43
...
- `shell/tests/test_providers.py`: 19
...
60 modules, 16808 lines of source, 180 public functions, 15 public classes, 805 test functions.
